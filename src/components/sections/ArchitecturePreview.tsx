...
const accent: Record<string, string> = {
  blue: "border-blue-400/30 bg-blue-500/10 text-blue-500",
  teal: "border-blue-500/30 bg-blue-500/10 text-blue-500",
      ...
      <Container className="mt-14">
          ...
          {layers.map((layer, i) => {
            ...
            return (
              <motion.li
                ...
                className="group relative flex items-center gap-5 rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-colors hover:border-blue-500/40 hover:shadow-md"
              >
                <div
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-baseline gap-x-3">
                    <h3 className="text-h4 font-semibold text-gray-900">{layer.name}</h3>
                    <span className="text-caption text-gray-400 font-mono">{layer.tech}</span>
                  </div>
                  <p className="mt-1 text-body-sm text-gray-600">{layer.description}</p>
                </div>
                <ArrowRight
                  className="hidden h-4 w-4 flex-none text-gray-500 transition-all group-hover:translate-x-0.5 group-hover:text-blue-400 md:block"
                  aria-hidden
                />
              </motion.li>
            );
        ...
        <div className="mt-10 text-center">
          <Link
            ...
            className="inline-flex items-center gap-2 text-body-sm font-semibold text-blue-500 transition-colors hover:text-blue-500"
          >
